...
public class UnitSlotItem : MonoBehaviour, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public event Action<UnitStat> OnLeftClicked; // 마우스 왼쪽 버튼 클릭 이벤트
    ...
    public void OnPointerClick(PointerEventData eventData) // 유닛 슬롯을 마우스 클릭 시 실행되는 함수
    {
    ...
    public void OnBeginDrag(PointerEventData eventData) // 유닛 슬롯을 드래그 시작 시 실행되는 함수
    {
        if (assignedUnit == null)
            return;
    ...
    public void OnDrag(PointerEventData eventData) // 유닛 슬롯을 드래그 진행 시 실행되는 함수
    {
        if (assignedUnit == null)
            return;
    ...
    public void OnEndDrag(PointerEventData eventData) // 유닛 슬롯을 드래그 완료 시 실행되는 함수
    {
        if (assignedUnit == null)
            return;
    ...
    public void SetEquippedState(bool isEquipped) // 유닛의 장착 상태 텍스트를 설정하는 함수
    {
        enterText?.gameObject.SetActive(isEquipped);
    }

    public void SetSelectedState(bool isSelected) // 선택된 유닛 슬롯 상태 패널을 설정하는 함수
    {
