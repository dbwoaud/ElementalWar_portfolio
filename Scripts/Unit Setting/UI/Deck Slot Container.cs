...
public class DeckSlotContainer : MonoBehaviour
{
    ...
    public void InitializeSlotContainer() // 덱 슬롯 컨테이너를 초기화하는 함수
    {
        if (slotItems == null)
            return;
    ...
    private void HandleSlotDropped(int index, UnitStat stat) // 덱 슬롯에 마우스 드롭 시 실행되는 함수
    { 
        OnSlotDropped?.Invoke(index, stat); 
    }

    private void HandleSlotSwapped(int from, int to) // 덱 슬롯 교체 시 실행되는 함수
    { 
        OnSlotSwapped?.Invoke(from, to); 
    }

    private void HandleBeginDrag(DeckSlotItem item) // 덱 슬롯에 마우스 드래그 시작 시 실행되는 함수
    { 
        OnBeginDragEvent?.Invoke(item); 
    }

    private void HandleDrag(PointerEventData data) // 덱 슬롯에 마우스 드래그 진행 시 실행되는 함수
    {
        OnDragEvent?.Invoke(data); 
    }

    private void HandleEndDrag(DeckSlotItem item, PointerEventData data) // 덱 슬롯에 마우스 드래그 완료 시 실행되는 함수
    { 
        OnEndDragEvent?.Invoke(item, data); 
    }

    public void UpdateDeckSlotUI(int index, UnitStat stat) // 슬롯 UI를 업데이트하는 함수
    {
