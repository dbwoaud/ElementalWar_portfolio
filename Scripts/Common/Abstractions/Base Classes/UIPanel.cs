...
public abstract class UIPanel : MonoBehaviour
{
    ...
    protected abstract void RegisterListener(); // UI 리스너를 등록하는 함수

    protected virtual void UnregisterListener() { } // UI 리스너를 해제하는 함수

    protected abstract void ResetUI(); // UI를 리셋하는 함수

    public virtual void Show() // 패널을 활성화하는 함수
    {
    ...
    private IEnumerator AnimateToVisible() // 패널을 활성화하는 코루틴
    {
    ...
    public virtual void HideImmediate() // 패널을 즉시 비활성화하는 함수
    {
        if (canvasGroup == null)
            canvasGroup = GetComponent<CanvasGroup>();
    ...
    private void StopAnimationCoroutine() // 현재 애니메이션 코루틴을 중지하는 함수
    {
